//! Packet capture engine and telemetry runner.
...
impl CaptureBuilder {
    ...
    pub fn build(self) -> Result<CaptureSession, CaptureError> {
        if let Some(file_path) = self.pcap_file {
            ...
            return Ok(CaptureSession::File(cap));
        }
        ...
        Ok(CaptureSession::Live(cap, self.bpf_filter))
    }
}
    ...
    pub fn spawn_worker(mut self, tx: mpsc::Sender<L2Packet>) -> tokio::task::JoinHandle<()> {
        tokio::task::spawn_blocking(move || {
            ...
            match &mut self {
                ...
                CaptureSession::File(cap) => {
                    info!("Telemetry reading offline pcap file");
                    while let Ok(packet) = cap.next_packet() {
                        Self::process_packet_data(packet.data, &mut stream_buffer, &mut codec, &tx);
                    }
                }
            }
            info!("Telemetry capture worker finished");
        })
    }
