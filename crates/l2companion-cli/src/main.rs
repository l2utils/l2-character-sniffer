...
use l2companion_capture::{list_devices, CaptureBuilder};
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Sniff {
            ...
            info!("Starting Lineage 2 Telemetry...");
            ...
            println!("Telemetry is running. Waiting for game packets... (Press Ctrl+C to stop)");

            // Live event display loop
            while let Ok(event) = event_rx.recv().await {
                match event {
                    CompanionEvent::CharacterLoaded(c) => {
                    ...
                    CompanionEvent::VitalsChanged { object_id, vitals } => {
                    ...
                    CompanionEvent::LocationChanged { object_id, location } => {
