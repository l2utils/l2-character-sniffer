...
Welcome to **`l2-character-sniffer`**! This repository is a modular Rust monorepo designed to sniff, decode, and track Lineage 2 network traffic, parse character states, and expose live telemetry over GraphQL, WebSockets, and REST.
...
└── crates/
    ├── l2-sniffer-protocol/        # Pure Rust packet codecs, opcodes, crypto, binary parsers
    ├── l2-sniffer-capture/         # Npcap/libpcap engine, device discovery, pcap/stream ingestion
    ├── l2-sniffer-core/            # Domain models, character state tracking, GraphQL & Axum API
    └── l2-sniffer-cli/             # Interactive CLI runner, device selector, terminal dashboard
```

### Dependency Invariant & Layering Rules
* **`l2-sniffer-protocol` (Pure Rust)**:
...
* **`l2-sniffer-capture` (Ingestion Layer)**:
  - Depends on `l2-sniffer-protocol`, `pcap`, and `pcap-file`.
...
* **`l2-sniffer-core` (Domain & Telemetry Engine)**:
  - Depends on `l2-sniffer-protocol`.
  - Maintains `CharacterTracker` state across multiple clients and accounts.
  - Exposes `SnifferEvent` broadcast stream, `async-graphql` schema, and Axum REST / WebSocket endpoints.
* **`l2-sniffer-cli` (Application Entry Point)**:
  - Connects `l2-sniffer-capture` to `l2-sniffer-core`.
...
cargo test -p l2-sniffer-protocol
cargo test -p l2-sniffer-core
cargo test -p l2-sniffer-capture
```
...
* When adding support for a new packet type:
  1. Add the opcode to [`ServerOpcode`](crates/l2-sniffer-protocol/src/opcode.rs).
  2. Implement the payload struct and parse function in [`packet.rs`](crates/l2-sniffer-protocol/src/packet.rs).
  3. Add a test case with a raw byte fixture in `packet.rs` tests.
  4. Update `CharacterTracker` in [`l2-sniffer-core`](crates/l2-sniffer-core/src/state.rs) to process the new packet and dispatch corresponding `SnifferEvent`s.
